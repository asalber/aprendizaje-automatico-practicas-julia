...
a.  Crear una nueva columna con la variable `Balance` con dos posibles categorías: `positivo` si ha habido beneficios y `negativo` si ha habido pérdidas.
...
a.  Obtener el número de valores perdidos en cada columna.

a.  Imputar los valores perdidos del alcohol con la media de los valores no perdidos para cada tipo de vino.

a.  Crear la variable categórica `Envejecimiento` recodificando la variable `meses.barrica` en las siguientes categorías.
...
a.  Crear la variable categórica `Dulzor` recodificando la variable `azucar.residual` en las siguientes categorías.
...
a.  ¿Cuáles son los 10 mejores vinos tintos crianza secos?
:::